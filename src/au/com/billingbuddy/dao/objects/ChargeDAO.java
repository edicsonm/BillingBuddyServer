...
//					stripeChargeVO.setBrand(resultSet.getString("Bin_Brand"));
	...
	public ArrayList<ChargeVO> searchChargesByDayFilter(ChargeVO chargeVO) throws ChargeDAOException {
		...
		ArrayList<ChargeVO> list = null;
		try {
			pstmt = connection.prepareCall("{call "+ConfigurationSystem.getKey("schema")+".PROC_SEARCH_CHARGES_BY_DAY_FILTER( ?, ?, ?, ?, ?, ?, ?, ?, ?)}");
			
			pstmt.setString(1, chargeVO.getCardVO().getNumber());
			pstmt.setString(2, chargeVO.getTransactionVO().getMerchantId());
			pstmt.setString(3, chargeVO.getCardVO().getBrand());
			
			pstmt.setString(4, chargeVO.getCardVO().getCountry());
			pstmt.setString(5, chargeVO.getCurrency());
			pstmt.setString(6, chargeVO.getMatch());
			...
			if (resultSet != null) {
				...
				while (resultSet.next()) {
					...
					chargeVO.getCardVO().setFunding(resultSet.getString("Card_Funding"));
					chargeVO.getCardVO().setLast4(resultSet.getString("Card_Last4"));
					chargeVO.getCardVO().setCountry(resultSet.getString("Card_Country"));
	...
	public ArrayList<ChargeVO> searchChargesRefundedByCustomer(ChargeVO chargeVO) throws ChargeDAOException {
		Connection connection = this.connection;
		ResultSet resultSet = null; 
		PreparedStatement pstmt = null;
		ArrayList<ChargeVO> list = null;
		try {
			pstmt = connection.prepareCall("{call "+ConfigurationSystem.getKey("schema")+".PROC_SEARCH_CHARGES_REFUNDED_BY_CUSTOMER( ? )}");
			
			pstmt.setString(1, chargeVO.getCardVO().getCustomerId());
